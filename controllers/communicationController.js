...
async function get(req,res,next){try{res.json({success:true,data:await service.get(req.params.communicationId)});}catch(e){next(e);}}
...
async function update(req,res,next){try{res.json({success:true,data:await service.update(req.params.communicationId,req.body)});}catch(e){next(e);}}
