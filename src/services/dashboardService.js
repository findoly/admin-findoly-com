...
async function getDashboard() {
  ...
  const providerWorkload = providers.map((provider) => ({
    provider,
    activeCount: enquiries.filter((enquiry) => enquiry.assignedProviderId === provider.id && !['completed', 'cancelled', 'lost'].includes(enquiry.status)).length
  })).sort((a, b) => b.activeCount - a.activeCount).slice(0, 8);
...
async function getNavigationCounts() {
  ...
  try {
    const [statusGroups, providerCount, openFollowUps, invoiceCount] = await Promise.all([
