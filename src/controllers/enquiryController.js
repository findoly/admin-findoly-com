...
async function showEdit(req, res, next) {
  try {
    ...
    if (!enquiry) return res.status(404).render('errors/404', { title: 'Enquiry not found' });

    const [categories, templates, sourceWebsites, providers, recommendations, templateCompletion] = await Promise.all([
      catalogService.listCategories({ active: true }),
      catalogService.listTemplates({ active: true }),
      catalogService.listSourceWebsites(),
      providerService.listProviders(),
      providerService.recommendProviders(enquiry),
      enquiryService.getTemplateCompletion(enquiry)
    ]);

    res.render('enquiries/edit', {
      title: `Edit ${enquiry.id}`,
      subtitle: 'Full enquiry edit',
      enquiry,
      categories,
      templates,
      sourceWebsites,
...
async function create(req, res, next) {
  try {
    const fields = collectDynamicFields(req.body);
    const enquiry = await enquiryService.createEnquiry({ ...req.body, fields }, req.admin.email);
    res.redirect(`/enquiries/${enquiry.id}?flash=Enquiry created`);
  } catch (error) {
    next(error);
  }
}
...
async function show(req, res, next) {
  try {
    const enquiry = await enquiryService.getEnquiry(req.params.id);
    if (!enquiry) return res.status(404).render('errors/404', { title: 'Enquiry not found' });
    const [providers, recommendations, followUps, communications, categories, templateCompletion] = await Promise.all([
      providerService.listProviders(),
      providerService.recommendProviders(enquiry),
...
async function update(req, res, next) {
  try {
    const hasDynamicFieldInput = Object.keys(req.body || {}).some((key) => key.startsWith('field__')) || req.body.dynamicFieldKeys;
    const input = { ...req.body };
    if (hasDynamicFieldInput) {
      input.fields = collectDynamicFields(req.body, true);
    }
    ...
    res.redirect(`${backTo}?flash=Enquiry updated`);
  } catch (error) {
    next(error);
  }
}
...
async function updateFields(req, res, next) {
  try {
    const fields = collectDynamicFields(req.body, true);
    await enquiryService.updateDynamicFields(req.params.id, fields, req.admin.email);
    res.redirect(`/enquiries/${req.params.id}?flash=Dynamic fields updated`);
  } catch (error) {
    next(error);
  }
}
...
async function assign(req, res, next) {
  try {
    await enquiryService.assignProvider(req.params.id, req.body.providerId, req.admin.email);
    res.redirect(`/enquiries/${req.params.id}?flash=Provider assigned`);
  } catch (error) {
    next(error);
  }
}
...
module.exports = { index, queue, showCreate, showEdit, create, show, update, updateFields, assign, note };
