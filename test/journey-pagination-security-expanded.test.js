...
const journeyCases = [
  ["journey contains the three approved ordered stages", () => assert.deepEqual(LEAD_JOURNEY, ["new", "verification", "approved"])],
  ...
  ["journey defaults former in_progress alias to new", () => assert.equal(canonicalLeadStatus("in_progress"), "new")],
  ["journey defaults former completed alias to new", () => assert.equal(canonicalLeadStatus("completed"), "new")],
  ["journey defaults former closed alias to new", () => assert.equal(canonicalLeadStatus("closed"), "new")],
  ["journey keeps provider conversion separate from CRM status", () => assert.equal(canonicalLeadStatus("sale_converted"), "new")],
  ["journey defaults former distributed status to new", () => assert.equal(canonicalLeadStatus("distributed"), "new")],
  ...
  ["journey rejects next after approved final stage", () => expectStatus(() => resolveLeadStatusTransition("approved", { action: "next" }), 400, /final journey stage/)],
  ...
  ["journey alias table contains only verification aliases", () => assert.deepEqual(Object.keys(STATUS_ALIASES).sort(), ["verification_pending", "verified"])],
];
