...
test("CRM Provider action client logs safe endpoint diagnostics and preserves Provider response status", async () => {
  const keys = ["CRM_PROVIDER_ACTION_API_URL", "CRM_PROVIDER_ACTION_API_TOKEN", "CRM_PROVIDER_ACTION_API_TIMEOUT_MS"];
  const previous = Object.fromEntries(keys.map((key) => [key, process.env[key]]));
  const previousFetch = global.fetch;
  const token = "crm-provider-action-secret-that-must-not-be-logged";
  Object.assign(process.env, {
    CRM_PROVIDER_ACTION_API_URL: "https://provider.findoly.com/api/internal/whatsapp/lead-unlock?ignored=1",
    CRM_PROVIDER_ACTION_API_TOKEN: token,
    CRM_PROVIDER_ACTION_API_TIMEOUT_MS: "5000",
  });
  const infoLogs = [];
  const errorLogs = [];
  const originalInfo = console.info;
  const originalError = console.error;
  console.info = (entry) => infoLogs.push(entry);
  console.error = (entry) => errorLogs.push(entry);
  global.fetch = async () => ({
    ok: true,
    status: 200,
    async json() {
      return { success: true, data: { status: "lead_unavailable", code: "LEAD_NOT_AVAILABLE" } };
    },
  });
  try {
    const absolute = require.resolve("../services/integration/provider-action-service");
    delete require.cache[absolute];
    const service = require(absolute);
    const result = await service.unlockLead({
      providerId: "provider-1",
      enquiryId: "enquiry-1",
      providerWhatsapp: "919999999999",
      communicationId: "communication-1",
      inboundMessageId: "inbound-1",
      idempotencyKey: "whatsapp-action-1",
      requestId: "request-1",
    });
    assert.equal(result.status, "lead_unavailable");
    const started = infoLogs.find((entry) => entry.event === "provider_action_request_started");
    const completed = infoLogs.find((entry) => entry.event === "provider_action_request_completed");
    assert.equal(started.endpointHost, "provider.findoly.com");
    assert.equal(started.endpointPath, "/api/internal/whatsapp/lead-unlock");
    assert.equal(started.configuredCredentialFingerprint.length, 12);
    assert.equal(completed.httpStatus, 200);
    assert.equal(completed.providerStatus, "lead_unavailable");
    assert.equal(completed.providerCode, "LEAD_NOT_AVAILABLE");
...
test("CRM Provider action client preserves HTTP error diagnostics and classifies timeouts", async () => {
  const keys = ["CRM_PROVIDER_ACTION_API_URL", "CRM_PROVIDER_ACTION_API_TOKEN", "CRM_PROVIDER_ACTION_API_TIMEOUT_MS"];
  const previous = Object.fromEntries(keys.map((key) => [key, process.env[key]]));
  const previousFetch = global.fetch;
  Object.assign(process.env, {
    CRM_PROVIDER_ACTION_API_URL: "https://provider.findoly.com/api/internal/whatsapp/lead-unlock",
    CRM_PROVIDER_ACTION_API_TOKEN: "c".repeat(48),
    CRM_PROVIDER_ACTION_API_TIMEOUT_MS: "5000",
  });
  const originalInfo = console.info;
  const originalError = console.error;
  ...
  try {
    ...
    global.fetch = async () => ({
      ...
      status: 401,
      async json() {
        return { success: false, code: "PROVIDER_ACTION_UNAUTHORIZED", message: "Unauthorized", requestId: "provider-request-1" };
      },
    });
    await assert.rejects(
      service.unlockLead({ providerId: "provider-1", enquiryId: "enquiry-1", inboundMessageId: "inbound-1" }),
      (error) => Boolean(
        error.code === "PROVIDER_ACTION_UNAUTHORIZED"
          && error.providerHttpStatus === 401
          && error.requestId,
      ),
    );
    assert.ok(errorLogs.some((entry) => entry.failureType === "http_error" && entry.httpStatus === 401));
    ...
    await assert.rejects(
      service.unlockLead({ providerId: "provider-1", enquiryId: "enquiry-1", inboundMessageId: "inbound-2" }),
      (error) => error.code === "PROVIDER_ACTION_TIMEOUT" && error.status === 504,
    );
    assert.ok(errorLogs.some((entry) => entry.failureType === "timeout" && entry.errorCode === "PROVIDER_ACTION_TIMEOUT"));
  } finally {
    global.fetch = previousFetch;
    console.info = originalInfo;
    console.error = originalError;
...
test("provider-facing messages cover diagnostic business states without using unlock wording", () => {
  const actionService = loadWithStubs("services/communication/provider-whatsapp-action-service.js", {
    "../../models/Communication": {},
    "./communication-service": {},
    "../integration/provider-action-service": {},
  ...
  for (const status of [
    ...
    const message = actionService.responseMessage({ status });
    assert.doesNotMatch(message, /unlock/i);
    assert.match(message, /enquiry|credits|provider account/i);
  }
});
