...
async function getFormSchema(input = {}) {
  ...
  return {
    ...
    submitEndpoint: '/api/requirements',
    payloadExample: {
      sourceWebsite,
      categorySlug,
      formType,
      ...
      additionalDetails: Object.fromEntries((template?.fields || []).map((field) => [field.name, field.defaultValue || ''])),
      metadata: { sourcePage: 'landing page URL', consent: true },
...
function getFieldCompletion(enquiry, template) {
  const fields = enquiry?.additionalDetails || enquiry?.fields || {};
  const templateFields = template?.fields || enquiry?.fieldDefinitions || [];
